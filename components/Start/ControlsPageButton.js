...
export default function Page1({ScreenWidth,ActivePage,count}){
    ...
    return(
        <>
        <TouchableOpacity onPress={()=>{count((3 - ActivePage) * 300,3)}} style={[{position: 'absolute', bottom: 36, left: 32,}, ActivePage === 3 && {transform: [{ scale: 0 }]}]}>
        ...
        <TouchableOpacity onPress={()=>count(300, ActivePage + 1)} style={[{position: 'absolute', bottom: 36, right: 32},ActivePage === 3 && {transform: [{ scale: 0 }]}]}>
